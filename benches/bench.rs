...
fn loopback_interface() -> Option<getifs::Interface> {
  ...
  ift.iter().find(|ifi| ifi.flags().contains(getifs::Flags::LOOPBACK & getifs::Flags::UP)).cloned()
}
...
fn bench_getifs_interface_by_index(c: &mut Criterion) {
  ...
  c.bench_with_input(BenchmarkId::new("getifs::interface_by_index", idx), &idx, |b, idx| {
    b.iter(|| {
...
fn bench_getifs_interface_by_name(c: &mut Criterion) {
  ...
  c.bench_with_input(BenchmarkId::new("getifs::interface_by_name", name.clone()), &name, |b, name| {
    b.iter(|| {
...
fn bench_getifs_interfaces_and_addrs(c: &mut Criterion) {
  let ifi = loopback_interface().unwrap();
  c.bench_with_input(BenchmarkId::new("getifs::interfaces_and_addrs", ifi.name().clone()), &ifi, |b, ifi| {
    b.iter(|| {
      ifi.addresses().unwrap();
    })
  });
}
...
fn bench_getifs_interfaces_and_multicast_addrs(c: &mut Criterion) {
  let ifi = loopback_interface().unwrap();
  c.bench_with_input(BenchmarkId::new("getifs::interfaces_and_multicast_addrs", ifi.name().clone()), &ifi, |b, ifi| {
    b.iter(|| {
...
fn bench_network_interface_interface_by_index(c: &mut Criterion) {
  let idx = loopback_interface().map_or(0, |ifi| ifi.index());
  c.bench_with_input(BenchmarkId::new("network_interface::interface_by_index", idx), &idx, |b, idx| {
    b.iter(|| {
      network_interface::NetworkInterface::show().unwrap().into_iter().find(|ifi| ifi.index == *idx);
    })
  });
}
...
fn bench_network_interface_interface_by_name(c: &mut Criterion) {
  let name = loopback_interface().map_or(SmolStr::default(), |ifi| ifi.name().clone());
  c.bench_with_input(BenchmarkId::new("network_interface::interface_by_name", name.clone()), &name, |b, name| {
    b.iter(|| {
      network_interface::NetworkInterface::show().unwrap().into_iter().find(|ifi| ifi.name == *name);
    })
  });
}
...
criterion_group!(
  benches,
  bench_getifs_interfaces, bench_getifs_interface_by_index,
  bench_getifs_interface_by_name, bench_getifs_interface_addrs,
  bench_getifs_interfaces_and_addrs, bench_getifs_interfaces_and_multicast_addrs,
